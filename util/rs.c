...
int mongo_util_rs_set_slave_preferred(mongo_link *link, char **prefSlaves, int prefSlavesCount, char **errmsg TSRMLS_DC) {
	...
	link->slave = 0;
	mongo_server **current_slave = malloc(prefSlavesCount * sizeof(mongo_server));
	...
	while (possible_slave) {	
		if (mongo_util_server_get_readable(possible_slave TSRMLS_CC)) {
			for (i = 0; i < prefSlavesCount; ++i) {
				if (!strcmp(prefSlaves[i], possible_slave->label)) {
					current_slave[count++] = possible_slave;  //Add the readable ones to a new set
				}
			}
		}
	...
	if (count > 0) {
		srand(time(NULL));
		int rnd = rand() % count;
		if (current_slave[rnd] != link->server_set->master) {
			link->slave = current_slave[rnd];
		}
	} else {
